...
class BankInterfaceProcessPaymentMethodSpockTest extends SpockRollbackTestAbstractClass {
	...
	def 'success'() {
		...
		def newReference = BankInterface.processPayment(new BankOperationData(iban, "",  100.0, TRANSACTION_SOURCE, TRANSACTION_REFERENCE))
	...
	def 'redo an already payed'() {
		...
		def firstReference = BankInterface.processPayment(new BankOperationData(iban, "", 100.0, TRANSACTION_SOURCE, TRANSACTION_REFERENCE))

		when: 'when there is a second payment for the same reference'
		def secondReference = BankInterface.processPayment(new BankOperationData(iban, "", 100.0, TRANSACTION_SOURCE, TRANSACTION_REFERENCE))
